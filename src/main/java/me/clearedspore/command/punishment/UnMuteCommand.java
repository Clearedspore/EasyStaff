...
public class UnMuteCommand extends BaseCommand {
    ...
    public void onUnMute(CommandSender player, String targetName, @Optional String... reasonParts) {
        if (targetName == null) {
        ...
        if (unMuted) {
            player.sendMessage(CC.sendBlue("You have unmuted &f" + target.getName() + ""));
        } else {
            player.sendMessage(CC.sendRed("That player is not muted!"));
        }
    }
}