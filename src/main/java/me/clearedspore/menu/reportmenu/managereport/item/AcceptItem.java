...
public class AcceptItem extends Item {
    ...
    public AcceptItem(String reportID, ReportManager reportManager, JavaPlugin plugin) {
    ...
    public ItemStack createItem() {
        ...
        meta.setDisplayName(CC.sendGreen("Accept"));
        List<String> lore = new ArrayList<>();
        lore.add(CC.sendWhite(""));
        lore.add(CC.sendWhite("Click to accept " + reportID));
        lore.add(CC.sendWhite(""));
    ...
    public void onClickEvent(Player player, ClickType clickType) {
        new ReportReasonMenu(plugin, reportID, reportManager, ReportStatus.ACCEPTED).open(player);
    }
}
