...
public class BanItem extends Item {
    ...
    public BanItem(OfflinePlayer target, Player viewer, JavaPlugin plugin, PunishmentManager punishmentManager) {
        this.target = target;
        this.viewer = viewer;
        this.plugin = plugin;
    ...
    public ItemStack createItem() {
        ...
        if(punishmentManager.getAllBans(target).size() > 1){
            item.setAmount(punishmentManager.getAllBans(target).size());
        }

        meta.setDisplayName(CC.sendBlue("Bans"));
        List<String> lore = new ArrayList<>();
        lore.add(CC.send("&f-----------------"));
        lore.add(CC.sendWhite("Click to check " + target.getName() + "'s ban history"));
        lore.add(CC.send("&f-----------------"));
