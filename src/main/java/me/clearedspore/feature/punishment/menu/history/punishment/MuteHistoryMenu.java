...
public class MuteHistoryMenu extends PaginatedMenu {
    ...
    public MuteHistoryMenu(JavaPlugin plugin, OfflinePlayer target, Player viewer, PunishmentManager punishmentManager) {
        super(plugin);
        this.target = target;
        this.viewer = viewer;
        this.plugin = plugin;
    ...
    public void createItems() {
        ...
        if (mutes.isEmpty()) {
            ...
            meta.setDisplayName(CC.sendRed("No Mutes found"));
            List<String> lore = new ArrayList<>();
            lore.add(CC.sendWhite("This player has no mutes."));
            meta.setLore(lore);
            item.setItemMeta(meta);
            addItem(item);
        } else {
            for (Punishment punishments : mutes) {
    ...
    protected void onInventoryClickEvent(Player player, ClickType clickType, InventoryClickEvent event) {
        ...
        if (clickType == ClickType.RIGHT) {
            ...
            if (clickedItem != null && clickedItem.hasItemMeta()) {
                ...
                if (displayName.startsWith(CC.sendBlue("Mute: "))) {
                    String punishmentId = displayName.substring(displayName.indexOf("Mute: ") + 6).trim();
                    ...
                    new MuteHistoryMenu(plugin, target, viewer, punishmentManager).open(player);
                }
            }
        }
    }
